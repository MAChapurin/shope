...
export const useAuth = () => {
  ...
  const checkValidPasswordEquality = () => {
    if (values.password !== values.password_repeat) {
      onError(INPUT_NAMES.PASSWORD_REPEAT, VALIDATION_SETTING.PASSWORD_REPEAT_ERROR)
      setIsPending(false)
      return false
    }
    return true
  ...
  const handlerLogin = async () => {
    let token = ''
    try {
      token = await loginUser(removePropertyAndReturn(values, INPUT_NAMES.PASSWORD_REPEAT))
      return token
    } catch (error) {
      console.log(error)
      return null
  ...
  const handlerRegister = async () => {
    let token = ''
    try {
      token = await registerUser(removePropertyAndReturn(values, INPUT_NAMES.PASSWORD_REPEAT))
      return token
    } catch (error) {
      console.log(error)
      return null
  ...
  const onRegister = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    setIsPending(true)
    const isValidationFields = checkValidEmail() && checkValidPassword() && checkValidPasswordEquality()
